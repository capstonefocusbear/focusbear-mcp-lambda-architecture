...
export class ActivityLibraryController {
  ...
  async getRoutineSuggestions(
    @Query() getRoutineSuggestionsQueryParamDto: GetRoutineSuggestionsQueryParamDto,
    @Body() getRoutineSuggestionsDto: GetRoutineSuggestionsDto,
    @AuthContext() { user }: Passport,
  ) {
    return this.activityLibraryService.getActivitiesRelatedToUserGoals(
      getRoutineSuggestionsQueryParamDto,
