...
export class UserConsentService {
  ...
  async upsertUserConsent(userConsent: UpdateUserConsentDto, user_id: string) {
    ...
    if (consent_type === UserConsentTypes.TERMS_OF_SERVICE && !!consent_status) {
      await this.userRepository.orm.update({ id: user_id }, { has_consented_to_terms_of_service: true });
    }
    if (consent_type === UserConsentTypes.TERMS_OF_SERVICE && !consent_status) {
      await this.userRepository.orm.update({ id: user_id }, { has_consented_to_terms_of_service: false });
    }
