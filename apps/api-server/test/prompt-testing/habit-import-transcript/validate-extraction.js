module.exports = (output, context) => {
  ...
  const validRoutineTypes = ['morning', 'evening', 'break'];
  ...
  habits.forEach((habit, index) => {
    ...
    if (!habit.routineType || !validRoutineTypes.includes(habit.routineType)) {
      errors.push(
        `Habit ${index + 1}: invalid routineType "${habit.routineType}". Must be one of: ${validRoutineTypes.join(
          ', ',
        )}`,
      );
    }
    ...
    if (habit.estimatedDurationMinutes !== undefined) {
      if (
        typeof habit.estimatedDurationMinutes !== 'number' ||
        habit.estimatedDurationMinutes < 1 ||
        habit.estimatedDurationMinutes > 120
      ) {
    ...
    if (habit.category && !validCategories.includes(habit.category)) {
      errors.push(
        `Habit ${index + 1}: invalid category "${habit.category}". Must be one of: ${validCategories.join(', ')}`,
      );
    }
  });
