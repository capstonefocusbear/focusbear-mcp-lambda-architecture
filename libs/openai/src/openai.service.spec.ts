...
describe('OpenAIService', () => {
  ...
  describe('streamChatReply', () => {
    beforeEach(() => {
      ...
      service = new OpenAIService(
        {
          pushNotification: { apiKey: 'test-push-key' },
        } as any,
        SentryServiceMock as unknown as SentryService,
        { t: () => '' } as unknown as I18nService,
        promptCacheServiceMock as any,
    ...
    it('should stream chat reply successfully', async () => {
      const mockResponse = {
        send: jest.fn().mockResolvedValue(undefined),
      };
      ...
      await service.streamChatReply(mockResponse as any, [{ role: 'user', content: 'Hi' }] as any);
    ...
    it('should retry on error', async () => {
      ...
      jest.spyOn(Stream, 'PassThrough').mockReturnValue(mockStream as any);
      jest
        .spyOn(service as any, 'getOpenAIChatCompletionsStreaming')
        .mockRejectedValueOnce(new Error('API Error'))
        .mockResolvedValueOnce([]);

      await service.streamChatReply(mockResponse as any, [{ role: 'user', content: 'Hi' }] as any);

      expect(service.getOpenAIChatCompletionsStreaming).toHaveBeenCalledTimes(2);
    });
  });
