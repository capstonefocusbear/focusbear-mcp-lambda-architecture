  ...
  async createChatReply(messages: ChatCompletionRequestMessage[], language = 'English') {
    try {
      ...
      const defaultChat: ChatCompletionRequestMessage[] = [
        {
          role: 'system',
          content: `You are a ${language} speaking chatbot(don't mention that you are a chatbot) named Focus Bear helping people to be productive and achieve the goals they set out to achieve. You are restricted to talking about productivity and habits. Please greet the user briefly.`,
        },
      ];
        ...
        while (retryCount < 3) {
          try {
            const completions = await openai.createChatCompletion({
              model: 'gpt-3.5-turbo',
              messages: hasExistingChat ? messages : defaultChat,
              max_tokens: 100,
              temperature: 0.7,
              n: 1,
            });
